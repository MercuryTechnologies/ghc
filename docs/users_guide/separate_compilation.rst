...
.. ghc-flag:: -ohie ⟨file⟩
    :shortdesc: set the filename in which to put the extended interface (hie) file generated by -fwrite-ide-info
    :type: dynamic
    :category:

    The extended interface output may be directed to another file
    ``bar2/Wurble.hie`` with the option ``-ohie bar2/Wurble.hie``
    (not recommended).
